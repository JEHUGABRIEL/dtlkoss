                ...
                {items[o.id] && items[o.id].length > 0 && (
                  <div className="rounded-lg border border-white/10 bg-zinc-800/80 p-3">
                    ...
                    <div className="mt-2 divide-y divide-white/5">
                      {items[o.id].map((item) => (
                        <div key={item.id || item.item_id} className="flex items-center justify-between py-1.5 text-sm">
                          <span className="text-white/80">{item.qty}× {item.item_name || item.name}</span>
