...
import { CustomersSection } from '../admin/ListSections'

// ═══════════════════════════════════════════════════════════════════
//  COMPOSANT Navbar pour l'admin
// ═══════════════════════════════════════════════════════════════════
function AdminNavbar({ activeTab, onLogout, onToggleSidebar }) {
  const TAB_NAMES = {
    ...
    <header className="sticky top-0 z-20 border-b border-white/10 bg-ink/90 backdrop-blur-xl backdrop-saturate-150">
      <div className="flex items-center justify-between px-5 py-3 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-vt-red to-vt-red-dark shadow-sm shadow-vt-red/20">
    ...
    <div className="flex min-h-screen bg-ink text-white">
      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-vt-red/20">
            <BarChart3 size={20} className="text-vt-red" />
          </div>
          <div>
            <h1 className="font-display text-lg font-bold leading-tight">Dashboard</h1>
            <p className="font-mono text-[9px] uppercase tracking-wider text-white/35">Vitraco RCA</p>
          </div>
        </div>
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 font-display text-sm font-medium tracking-wide transition-all ${
                  isActive
                    ? 'bg-vt-red text-white shadow-lg shadow-vt-red/25'
